  ...
  function drawForNextHole() {
    ...
    setCardState((current) => {
      const next: GameCardState = {
        drawPileIds: [...current.drawPileIds], handIds: [...current.handIds], installedClubIds: [...current.installedClubIds], installedBallIds: [...current.installedBallIds], discardIds: [...current.discardIds],
      };
      const resolution = drawAtHoleStart(next);
      const drawnAction = resolution.actionIds[0] ? CARD_BY_ID.get(resolution.actionIds[0]) : null;
      const installed = [...resolution.installedClubIds.map((id) => CARD_BY_ID.get(id)?.name), ...resolution.installedBallIds.map((id) => CARD_BY_ID.get(id)?.name)].filter(Boolean);
      setMessage(drawnAction ? `Loch ${hole + 1} · ${drawnAction.name} gezogen` : installed.length ? `Loch ${hole + 1} · Equipment installiert: ${installed.join(", ")}` : `Loch ${hole + 1} · kein weiterer Draw`);
      return next;
    });
    setHole((value) => Math.min(6, value + 1));
  }

  return <main className="bw-world min-h-screen text-[#f5ecd5]" data-recovery-status="CARD_LAYER_RECOVERED" data-design-pass="emerald-world-pass-02">
    <EmeraldWorldSkin/>
    <div className="bw-page">
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-[220px_1fr]">
        ...
        <div className="grid gap-5">
          <section className="bw-panel p-4"><div className="flex items-center justify-between"><div><p className="bw-kicker">Equipment</p><h2 className="mt-1 font-semibold">Aktives Bag</h2></div><span className="text-xs text-moss">{installedClubs.length} Schläger · {installedBalls.length} Bälle</span></div><div className="bw-game-card-strip mt-4">{[...installedClubs, ...installedBalls].map((card) => <div className="bw-card p-2" key={card.id}><CardArtwork id={card.id} physicalNumber={card.physicalNumber} name={card.name} decorative/><p className="mt-2 truncate text-xs">{card.name}</p></div>)}</div></section>
          <section className="bw-panel p-4"><div className="flex items-center justify-between"><div><p className="bw-kicker">Deine Optionen</p><h2 className="mt-1 font-semibold">Aktionshand</h2></div><span className="text-xs text-moss">{hand.length} Karten</span></div><div className="bw-game-card-strip mt-4">{hand.map((card) => <div className="bw-card p-2" key={card.id}><CardArtwork id={card.id} physicalNumber={card.physicalNumber} name={card.name} decorative/><p className="mt-2 truncate text-xs">{card.name}</p></div>)}</div></section>
          <div className="flex justify-end"><button type="button" onClick={drawForNextHole} disabled={hole >= 6} className="bw-gold-button">{hole >= 6 ? "Loch 6 erreicht" : "Nächstes Loch · Draw"}</button></div>
        </div>
      </section>
