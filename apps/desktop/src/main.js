...
const mantle = new THREE.Mesh(geometry, new THREE.MeshPhysicalMaterial({
  color: 0x063c2c, transparent: true, opacity: 0.48, roughness: 0.2,
  metalness: 0.08, transmission: 0.15, emissive: 0x05271e, emissiveIntensity: 0.65
}));
scene.add(mantle);

const field = new THREE.Mesh(new THREE.SphereGeometry(0.58, 48, 48), new THREE.MeshBasicMaterial({
  color: 0x16a46b, transparent: true, opacity: 0.32
}));
field.scale.set(0.84, 1, 0.72);
scene.add(field);

const meridianCurve = new THREE.CatmullRomCurve3([
  new THREE.Vector3(-0.55,-0.38,0.52), new THREE.Vector3(-0.2,0.22,0.72),
...
const presets = {
  IDLE:[0.38,0.18,0.12,0.20], SPEECH_DETECTED:[0.74,0.62,0.48,0.28], LISTENING:[0.82,0.92,0.40,0.22],
...
function frame(now) {
  ...
  mantle.scale.set(breath, breath * (1 - 0.002 * Math.cos(t)), breath);
  field.scale.set(0.84 * (1 + aperture * 0.025), 1 + aperture * 0.025, 0.72);
  field.material.opacity += ((0.18 + presence * 0.34) - field.material.opacity) * 0.06;
  field.rotation.y += 0.0008 + flow * 0.0015;
  meridian.material.opacity = 0.4 + gold * 0.6;
