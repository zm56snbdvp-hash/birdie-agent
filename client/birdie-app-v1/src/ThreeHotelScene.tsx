...
export function ThreeHotelScene() {
  ...
  useEffect(() => {
    ...
    // Distant rolling golf landscape.
    ...
    // Putting patch and flag: decorative only, no new V1 interaction.
    ...
    // Simulator bay inspired by the visual brand imagery.
    ...
    // Terrace deck, small tables, and warm string lights.
    ...
    // Minimal golf bag silhouette at the arrival side.
    ...
    // Fireflies / golden dust to soften the scene at dusk.
    ...
    const animate = () => {
      ...
      avatar.position.x = THREE.MathUtils.clamp(avatar.position.x + dx, -8.2, 9.1);
      avatar.position.z = THREE.MathUtils.clamp(avatar.position.z + dz, 0.7, 9.2);
      ...
      avatar.rotation.y += rotationDelta * Math.min(1, dt * 9);
      ...
      const stride = moving ? Math.sin(walkPhase) * 0.48 : 0;
      leftLegPivot.rotation.x += (stride - leftLegPivot.rotation.x) * 0.22;
      rightLegPivot.rotation.x += (-stride - rightLegPivot.rotation.x) * 0.22;
      leftArmPivot.rotation.x += (-stride * 0.72 - leftArmPivot.rotation.x) * 0.22;
      rightArmPivot.rotation.x += (stride * 0.72 - rightArmPivot.rotation.x) * 0.22;
      torso.position.y = 1.35 + (moving ? Math.abs(Math.sin(walkPhase * 2)) * 0.035 : Math.sin(elapsed * 1.4) * 0.012);

      // Slow camera follow keeps the tiny world feeling toy-like rather than FPS-like.
      const cameraTargetX = avatar.position.x * 0.22;
      const cameraTargetZ = 15.2 + Math.max(0, avatar.position.z - 7) * 0.12;
      camera.position.x += (cameraTargetX - camera.position.x) * 0.032;
      camera.position.z += (cameraTargetZ - camera.position.z) * 0.025;
      camera.lookAt(avatar.position.x * 0.16, 2.15, -1.25);
