...
test("desktop is a fullscreen estate and all three function tabs return to Birdie", async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveTitle("BirdieWorld – Immersive Estate V0.3.4");
  await expect(page.locator("html")).toHaveAttribute("lang", "de");
  await expect(page.getByRole("heading", { level: 1, name: "BirdieWorld Immersive Estate V0.3.4" })).toBeAttached();
  const main = page.locator("main");
  await expect(main).toHaveAttribute("data-immersive-estate", "birdieworld-immersive-estate-v0.3.4");
...
test("forced WebGL fallback keeps the whole estate, NPCs and function menu operable", async ({ page }, testInfo) => {
  ...
  const fallback = page.locator("[data-estate-fallback='birdieworld-immersive-estate-v0.3.4']");
